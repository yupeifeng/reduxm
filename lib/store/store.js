   ...
   */
		...
		var _loop2 = function _loop2(key) {
			...
			if (storeActionTypeSign[key]) {
				...
				var storeLogsSignKey = storeLogsSign[key];
				var storePropsSignKey = storeActionTypeSign[key];
				var _storeComputedSignTemp2 = storeComputedSign;
				var _storeLogsSignTemp2 = storeLogsSign;
				...
				actions[storeName + '_' + storeActionTypeSign[key]] = function (state, action) {
					...
					//重新得到计算者
					for (var computedKey in target) {
						if (_storeComputedSignTemp2[computedKey] && key == _storeComputedSignTemp2[computedKey]) {
							arg[computedKey] = target[computedKey].apply(action);
							//埋入日志输出点,便于使用人员定位数据流向
							if (_storeLogsSignTemp2[computedKey]) {
								switch (_storeLogsSignTemp2[computedKey]) {
									case 'warn':
										console.warn('---storeComputed:' + computedKey + '---  \n ---dependency:' + _storeComputedSignTemp2[computedKey] + '---');
										console.warn(action[key]);
										break;
									case 'log':
										console.log('---storeComputed:' + computedKey + '---  \n ---dependency:' + _storeComputedSignTemp2[computedKey] + '---');
		...
		_actiontypefactory2.default.initActionType(storeName, actionTypes);
		//按名称将数据、数据改变函数存入ReducerFactory
		_storefactory2.default.initStore(storeName, initialData, excludeData, actions);
	...
	return Object.assign({}, _reducerfactory2.default.getAllInitData(storeName), _storefactory2.default.getAllInitData(storeName));
}, _temp);
